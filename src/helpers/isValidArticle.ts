...
const INVALID_SUMMARY_PATTERNS: readonly RegExp[] = [
  ...
  /^[＊*].*(?:削除|白紙化|存在していません).*[＊*]$/u,
  new RegExp(
    `(?:転送済み|転送記事化(?:${PUNCTUATION_PATTERN}|$)|白紙化.*転送|白紙化.*新規投稿|削除要請中|削除待ち)`,
    'u'
  ),
  new RegExp(
  ...
  /^(?:内容|記事内容|記事本文)の削除/u,
  new RegExp(
    `(?:白紙化${VERB_SUFFIX_PATTERN}|白紙化に${VERB_SUFFIX_PATTERN}|白紙化された記事(?:です)?|白紙化記事(?:です)?|白紙化した記事(?:です)?|白紙化による記事です|白紙化となりました|白紙化が決定しました|白紙化を行いました|白紙化がおこなわれ(?:ました|ています)|白紙化処理|白紙化済)$`,
    'u'
  ),
